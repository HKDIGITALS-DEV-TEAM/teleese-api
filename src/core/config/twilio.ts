...
class TwilioService {
  ...
  public async generatePhoneNumber(countryCode: string): Promise<string> {
    ...
    try {
      ...
      logger.info(`cout de la génération du numéro ${phoneNumber} : ${cost}`);
  ...
  public async registerPhoneNumber(phoneNumber: string): Promise<void> {
    try {
      ...
      if (existingNumbers.length === 0) {
        ...
        logger.info(`Successfully registered ${phoneNumber} with Twilio`);
      } else {
        incomingNumber = existingNumbers[0];
        logger.info(`${phoneNumber} already exists on Twilio`);
      }
      ...
      logger.info(`Successfully configured ${phoneNumber}`);
      const endBalanceValue = parseInt(
        (await this.getCurrentBalance()).balance
      );
      const cost = this.calculateTwilioCost(startBalanceValue, endBalanceValue);

      logger.info(`cout de l'enregistrement du numéro ${phoneNumber} : ${cost}`);
    } catch (error) {
      logger.error(`Error registering ${phoneNumber}:`, error);
      throw error;
    }
  }
